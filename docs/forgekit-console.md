...
2. **non-raster(VS Code 등)** → **같은 픽셀 아이콘의 BRAILLE 렌더**(`halfblock.render_braille`,
   2×4 dots/cell = half-block 대비 ~8× 해상도, autocontrast + threshold, **12 cols ≈ 6행**). 렌더
   spike 결과 — grayscale half-block(2×1)보다 헤드폰/머리/얼굴 figure 가 또렷해 채택. braille 이
   안 되면 grayscale half-block 으로 degrade. (opt-in `FORGEKIT_AVATAR=portrait` 는 grayscale
   half-block 유지.) 즉
