...
운영 CLI(`/provider budget`, `commands/router.py` → `provider_surface.apply_set_budget`/`budget_lines`):

- `/provider budget <id> <limit>` — id(built-in) + 정수 limit 검증 → `set_provider_budget` 영속(`~/.forgekit/config.json`). `0` 이하 = 한도 해제(unbounded, 0 으로 invent 안 함). 빈/잘못된 브레인은 persist 가 거부 → budget 은 기존 브레인의 ring-fencing.
- `/provider budget show`(또는 인자 없는 `/provider budget`) — 설정 한도 + **오늘** usage ledger 의 spent/over 를 정직 표기(한도 없는 provider 는 표시 안 됨 = unbounded). router 는 얇게 유지(render/CRUD), spent 계산·표기 로직은 provider 패키지.
- unknown id / non-integer / missing limit 은 `KIND_ERROR`(silent no-op 없음).

회귀 `tests/forgekit/test_provider_budget.py`(enforcement) · `tests/forgekit/test_provider_budget_cli.py`(CLI 표면),
evidence `examples/provider-budget/`(`provider-budget-evidence.txt` enforcement · `cli.txt` CLI).
