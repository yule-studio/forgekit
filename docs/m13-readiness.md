...
| G-M12-01 | M12 / supervisor self-improvement detect+dispatch hook | M12 커밋 `d083446` 에서 `run_supervisor_watch_loop` 가 `self_improvement_detect_fn` / `self_improvement_dispatch_fn` / `self_improvement_interval_seconds` 3 인자를 받고, `_run_self_improvement_tick` 이 detect → `plan_self_improvement_proposal` → dispatch 로 묶임. `agents/lifecycle/self_improvement.py` 에 proposal planner + 위험 verb L3/L4 escalation + `propose_runtime_code_change_stub` 추가. 단위 테스트 23 추가 (test_self_improvement.py 17 + test_supervisor_status_post.py 6). | **CLOSED at lib level** / 미완 at runtime level | hook+planner 자체는 합류. 단 `runtime/run_service.py` `_run_async` supervisor 분기는 여전히 status_post_fn 만 넘기고 self-improvement 3 인자를 **None** 인 채 호출 → production 스케줄러는 아직 tick 안 함. M13 첫 후속 커밋에서 supervisor 분기에 `senior_agent.emit_self_improvement_proposal` 기반 detect/dispatch + 간격(예: 600s) 을 묶어줘야 라이브 e2e 검증 가능. **분류: P1 (M13 in-scope, 작은 wiring 한 줄).** |
...
- **Conditional**:
  1. **G-M12-01 (runtime level)** — `runtime/run_service.py` `_run_async` supervisor 분기에서 `self_improvement_detect_fn` / `self_improvement_dispatch_fn` / `self_improvement_interval_seconds` 3 인자를 아직 안 넘긴다. M13 첫 후속 커밋에서 `senior_agent.emit_self_improvement_proposal` 기반 detect/dispatch 와 간격(예: 600s) 를 묶어야 production 스케줄러가 실제로 tick 하고 라이브 회귀 §7 시나리오 F (self-improvement proposal 자동 작성) 가 검증 가능해진다.
