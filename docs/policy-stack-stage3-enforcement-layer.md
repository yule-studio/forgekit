...
| `src/yule_orchestrator/agents/coding/tracking_enforcement.py` | C | validate_tracking_chain. |
| `src/yule_orchestrator/agents/coding/pr_slice_classifier.py` | C | classify_pr_slice + recommend_split. |
| `src/yule_orchestrator/agents/lifecycle/growth_ledger.py` | C | GrowthEvent + capture + promotion 후보. |
| `src/yule_orchestrator/agents/job_queue/vault_push_dispatcher.py` | C | VaultPushRequest + dispatch (mode-aware). |
| `src/yule_orchestrator/agents/design/asset_routing.py` | C | validate_asset_name + SVG/raster boundary. |
| `src/yule_orchestrator/discord/engineering_channel_router.py` | U | `_persist_coding_session_context` 가 tracking_validation / growth_ledger 호출. |
| `src/yule_orchestrator/agents/lifecycle/session_status.py` | U | SessionStatusReport 에 신규 7 필드 surface. |
| `src/yule_orchestrator/discord/engineering_conversation.py` | U | format_status_diagnostic_response 의 신규 라인. |
