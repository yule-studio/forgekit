...
- provider별 연결 **생성(projection) ✅** — `scripts/sync_mcp_projection.py` 가
  `.mcp.json`(Claude) / `.codex-plugin/mcp.toml`(Codex) / `.gemini-plugin/mcp.json`(Gemini)
  를 생성(secret 값 없이 env 참조만, HOME 미터치). 상세 `integrations/mcp/README.md`.

## 6.5 projection 엔진 (✅ 코드 SSoT — 외부 tool 후보 → provider 생태계)

§1~6 이 *분류*를 prose 로 정의한다면, 본 절은 그 분류를 **deterministic 하게 실행**하는
코드 SSoT 다. 수집된 skill/plugin/tool 후보가 armory catalog 에만 머무르지 않고
Claude/Codex/Gemini/Ollama 각 생태계로 **projection / attach / connect** 되는 경로.

- **엔진**: `packages/forgekit-provider/src/forgekit_provider/projection/`
  (`models.py` = `ToolCandidate`/`ProjectionVerdict`/`TargetPlan` + taxonomy kind·capability
  class 어휘, `rules.py` = `project(candidate) → verdict`).
- **입력**: vendor-neutral `ToolCandidate(taxonomy_kind, capability_class, …)` — provider 이름
  없음(capability 가 lens). `taxonomy_kind` 은 §1 의 6축(backend/skill/hook/mcp/runtime_plugin/
  harness_projection).
- **출력**: `ProjectionVerdict` — `projection_targets`(claude/codex/gemini 부분집합)와
  `backend_role`(ollama)을 **분리된 필드**로 둔다. 절대 섞이지 않는다(`is_backend`).
- **규칙(요약)**: capability class → primary projection target([matrix §2](provider-capability-matrix.md)
  를 미러). 그 다음 taxonomy kind 가 제약: hook 은 Gemini 평면 없음(claude/codex만), MCP 는
  MCP-capable harness(claude/codex/gemini)만·Ollama 제외, backend·분류/요약/압축/fallback
  capability 는 Ollama backend slot(projection 아님), runtime_plugin 은 vendor-neutral(단일
  primary 없음).
- **attach/connect/verify 조건**: 각 target plan 이 *어떻게 붙고(attach)·무엇을 연결하며
  (connect)·어떻게 검증하나(verify)* 를 동반한다. 생성 connector 가 없는 (kind,target) 쌍은
  `has_connector=False` 로 **정직하게 manual 표시**(fake connector 금지).
- **operator 표면**:
  - `/setup` 하단 projection legend(claude/codex/gemini=projection · ollama=backend).
  - `/provider attach <도구id>` — 선택 tool 하나의 verdict 상세(target별 attach/connect/verify).
    surface: `packages/forgekit-provider-connect/.../attach.py`.
  - `/resolve <요청>` packet — 선택된 armory skill/weapon 별 compact projection block.
    bridge: `apps/forgekit-console/.../provider_projection.py`(catalog `capability_note`/
    category → vendor-neutral candidate 매핑, SSoT 는 중립 유지).
- **회귀**: `tests/forgekit/test_provider_projection.py`(Ollama 가 projection_targets 에
  새지 않음을 (kind×capability) 전수 검증 포함).
