...
| --- | --- | --- | --- |
| `handoff/gateway.py` (console) | `agents.product_intake.shaping` | intake packet bridge | `packages/agent-contracts` command |
| `forgekit_runtime.lifecycle...._bridge_troubleshooting` | `agents.lifecycle.troubleshooting_{ledger,record}` | 운영 메모리 capture (best-effort lazy, 없으면 no-op) | `packages/agent-contracts` event |
| `data/status_loader.py` (console) | `agents.harness.operator_surface` · `diagnostics.doctor` · `agents.job_queue.*` · `runtime.status` | 대시보드/doctor 데이터 | `agent-contracts` status |

> **handoff seam (해소):** runtime loop 의 intake→packet 의존은 더 이상 직접 import 가 아니다.
> `forgekit_runtime.runtime.loop.register_handoff_runner(fn)` 로 operator app(console shim)이
> 주입한다 — core 는 app 을 import 하지 않는다(packages→apps 유지). lifecycle→yule_engineering 은
> forgekit-runtime 으로 옮겨졌지만 best-effort lazy(import-clean)라 import-time app dep 아님.
...
| `policy/runtime_mode.py` | ~300 | runtime mode → 실 routing/budget/approval | `packages/forgekit-runtime` (현재 `forgekit_provider.policy.runtime_mode` 경유) | planned (provider→runtime 재배치) |
| `runtime/` (daemon/loop/autopilot_tick/heartbeat/runbook/surface) | 780 | always-on bounded daemon core | `packages/forgekit-runtime` (`forgekit_runtime.runtime`) | **done** (옛 경로 shim; handoff seam) |
| `autopilot/` | 1012 | safe-class autopilot core | `packages/forgekit-runtime` (`forgekit_runtime.autopilot`) | **done** (옛 경로 shim) |
| `lifecycle/` | 511 | 실패 escalation / 운영 메모리 bridge | `packages/forgekit-runtime` (`forgekit_runtime.lifecycle`) | **done** (옛 경로 shim; yule_engineering lazy debt) |
| `selfimprove/` | 190 | self-improvement loop | `packages/forgekit-runtime` (`forgekit_runtime.selfimprove`) | **done** (옛 경로 shim) |
| `notify/` | 336 | approval/alert inbox (operator action) | `packages/forgekit-runtime` (`forgekit_runtime.notify`) | **done** (옛 경로 shim) |
| `security/` | 239 | red/blue 계획(plan-only) | `packages/forgekit-runtime` (`forgekit_runtime.security`) | **done** (옛 경로 shim) |
...
> - `forgekit-runtime`(완) — `runtime/autopilot/lifecycle/notify/selfimprove/security`(6 dir) →
>   `forgekit_runtime.*`. cross-package dep 8곳 absolute 화(config/contracts/provider/nexus).
>   intake→packet **handoff 는 seam** 으로 분리(app 이 `register_handoff_runner` 로 주입) → core 는
>   app 을 import 안 함. 단 lifecycle 의 troubleshooting bridge 는 best-effort lazy yule_engineering
>   debt 로 잔존(WT4 에서 agent-contracts 로 역전). 검증: root CI 6590 OK, runtime guard 5 OK.
