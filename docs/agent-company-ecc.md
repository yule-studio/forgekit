...
| **Skill** | 없음 (역할 contract 안에 묻혀 있음) | `skills/<id>/SKILL.md` (디렉터리 + 4 섹션 본문) | **신설 — 단일 파일** | 가장 약한 영역. ECC 의 4 섹션 본문 + Yule 의 IO contract frontmatter 결합. | `skills/<id>.md` | dispatcher 미존재 → 정의만 있고 동작 안 한다는 오해. README 에 명시 |
...
| **Skill 본문 IO** | (해당 없음) | 본문이 IO schema 아닌 prose | **거부 → strengthening** | Yule skill 은 frontmatter 로 `input_contract` `output_contract` `preconditions` `side_effects` 강제 | `skills/README.md` §3 | — |
...
- 작성 가이드: [`skills/README.md`](../skills/README.md).
- 1 reference manifest: [`research-collect.md`](../skills/research-collect.md).
