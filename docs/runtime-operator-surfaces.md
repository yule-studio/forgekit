...
## 4.2 runtime readiness join — "always-on 이 진행 가능한가" 한 verdict (#428)

`runtime/readiness.py` 가 daemon heartbeat × goal continuity × **declared live transport** 를
join 해 진행 가능성의 *binding constraint* 를 단일 verdict 로 답한다(`/daemon` + `/setup` 공용):

- verdict: `setup_required`(브레인 미설정) · `idle_no_goals`(목표 없음) · `awaiting_operator`
  (승인 대기) · `no_live_lane`(active goal 은 있으나 live transport 없음 — packet 누적, **fake-live
  아님**) · `progressing`(active goal + live lane).
- transport 는 work slot(default_chat/execution/research)을 `routing.resolve_routing` 으로
  config 파생: claude/codex(CLI)는 routing-only, gemini/ollama 가 live-capable. **no fake-live** —
  probe 없으면 "선언된 live lane(미검증)", `live_map`(probe) 있으면 "검증됨"/False 면 정직 downgrade.
- unattended 는 platform-honest: macOS LaunchAgent=clamshell sleep suspend(`caffeinate -s`),
  Linux/systemd `--user`=logout 정지 가능(`loginctl enable-linger`) — `install-unit` 이 둘 다 생성.
- `/daemon`(`surface.daemon_status_lines`)은 goal-loop 라인을 이 readiness 블록으로 대체(상위집합),
  `/setup`(`bootstrap.bootstrap_lines`)은 probe-검증 live_map 으로 readiness 블록을 append → operator
  가 connect 뿐 아니라 continuity 를 한 화면에서 본다. 회귀 `test_runtime_readiness`, evidence
  `examples/runtime-readiness/`.

## 4.3 provider/runtime continuity — per-tick lane + budget (history trail)

§4.2 readiness 는 *현재* 진행 가능성(declared live transport)을 한 verdict 로 답한다. 본 절은 그
보완으로 **과거 tick 별** 진행 이력을 durable 하게 남긴다: always-on loop 의 각 tick 이 **어떤
provider lane 을 통해** 진행했는지·예산 안인지.
`runtime serve` 의 composed tick 은 `runtime/provider_continuity.with_provider_continuity` 로 래핑돼
매 tick:

1. **provider lane 해결** (`runtime/provider_lane.resolve_tick_lane`, EXECUTION slot) — `routing.resolve_routing`
   위의 얇은 read. 세 가지를 정직히 구분: **brain**(declared = 라우팅 참여자) vs **actual transport**
   (console live-submit 가능한 실제 provider = gemini/ollama) vs **fallback**(declared 불가 시 explicit
   fallback + 시도 chain). Claude/Codex 는 `unsupported_in_console` 이라도 **brain participant 로 유효**
   표기(라이브 lane 으로 위조하지 않음); gemini/ollama lane 이면 **live transport 로 정직** 표기.
2. **budget snapshot** (오늘 usage-ledger 합 vs `daily_token_budget`, 0=무한) + **receipt**(executed paths).
3. **durable 기록** — `runtime/tick_ledger.py` 가 tick 당 1 줄을 `~/.forgekit/state/runtime-tick-ledger.jsonl`
   에 append(best-effort, tick 을 절대 깨지 않음). `TickOutcome` 에 `provider_lane`/`budget` 를 실어
   heartbeat/status 가 표면.

표면: `forgekit runtime status` / `/daemon`(`runtime/surface.provider_lane_lines`)이 최근 N tick 의
`lane short · exec/blocked/waiting · budget` 연속성을 보여준다 — "내 long-running goal 이 어느 provider 로,
예산 안에서 계속 진행 중인가"를 momentary status 가 아니라 durable trail 로 답한다. 기록 없으면 "기록 없음"
(fake 진행 없음). 회귀 `test_provider_runtime_continuity`(lane 해결·per-tick 기록·**bounded serve 6/6 무정지
progression**), evidence `examples/runtime-continuity/continuity.txt`.

> 새 routing engine 을 만들지 않는다 — 기존 `routing.resolve_routing` + usage budget + ledger 패턴을
> tick 속성으로 surface 만 한다(ponytail anti-overbuild). lane B 는 실행을 바꾸지 않고 *관측/기록*만 더한다.

always-on loop 의 각 tick 이 **어떤 provider lane 을 통해** 진행했는지·예산 안인지 durable 하게 남긴다.
`runtime serve` 의 composed tick 은 `runtime/provider_continuity.with_provider_continuity` 로 래핑돼
매 tick:
