...
| 5 | `/goal` 중심 자기관리(수집·분석·실행·검증·기록) | ✅ core+tick+surface+execute+decompose/continuation+**autonomy(자율분해/재계획)** done (GW1·GW4·GW5·GW-EXEC·AUTONOMY) | GW1 모델/영속 + GW4-A goal-tick(수집→제안→linkage→evidence, 실행 0) + GW4-B execute bridge(승인된 packet→기존 게이트 실행 인가→execution+verification evidence) + GW5 `/goal` console surface + GW-EXEC `forgekit_goal.planning`(큰 goal→ordered child step 분해 + progress + continuation) + **AUTONOMY `goal_scheduler_tick`(serve 에 discovery 배선: ACTIVE goal 자동 packetize + ≥2 area 면 자율 분해) + replan policy(stuck→bounded retry→escalate, 사유 영속) + approval disposition(자율안전/승인필요 분리), `_build_tick_fn` 4-pass 전체 루프)** ✅ + **EXEC-CORE intent seed (`goal_intent.intent_packets`): repo gap 이 없는 intent goal 도 자기 title 에서 결정 레인 첫 step(PM brief)을 packet 으로 seed → `packets: 0` stuck 종료, risky seed 라 `awaiting_approval` 로 surface(자동 실행 아님)** ✅. evidence `examples/goal/{roundtrip,tick,surface,execute-bridge,planning,autonomy,autopilot-intent}.txt`. **남은 것:** 실제 물리 mutation 은 BoundedMutator 게이트(WT3, P2); PM brief→tech-lead decision 후속 step packet 화는 operator 가 brief 를 채운 뒤(현재는 첫 step 까지 자동). |
...
| 9 | 새 기능/패턴/provider capability 자기조사→backlog 승격 | ✅ promotion+**자율 분해/재계획(AUTONOMY)** done | discovery/repo-local 신호 → risk-classified packet → **goal 에 linkage + evidence 승격**(`goal_tick`) ✅. GW-EXEC `/goal plan` 수동 분해. **AUTONOMY:** `goal_scheduler_tick` 이 serve 에서 discovery 를 ACTIVE goal 에 자동 실행→packetize, ≥2 area 면 자율 분해, 막히면 replan(bounded retry→escalate). **남은 것:** 외부 connector(Figma/YouTube=P2). |
...
실행 순서(의존 기준): **GW1 ✅ → GW2-A ✅ → GW3 ✅ → GW4-A ✅ → GW4-B ✅ → GW6 ✅ → GW5 ✅ → GW7 ✅ → GW8 ✅ → GW-EXEC ✅ → AUTONOMY ✅** (merge-prep: [`forgekit-goal-mergeprep.md`](forgekit-goal-mergeprep.md)). 남은 seam: ~~GW4-B 실행 bridge~~ ✅(`execute_approved_packet`, 기존 게이트 재사용·execution+verification evidence·trailer stamp), ~~GW4-B 실제 물리 mutation~~ ✅(`apply_approved_packet` opt-in: safe-class 만 BoundedMutator bounded write + 실제 git commit + evidence→Nexus vault, 기본 경로 무변경; destructive 는 여전히 approval-gated), ~~in-console approve/deny UI~~ ✅(`/goal awaiting|approve|deny`, wave-2 gw3), ~~GW2-B trailer→identity~~ ✅(`check_identity_binding`, bridge commit message 에 trailer 자동 stamp), ~~큰 goal 분해/continuation(실행 teeth)~~ ✅(GW-EXEC: `forgekit_goal.planning` decompose+progress+continuation + `goal_continuation_tick` rollup→evidence-gated done, `/goal plan|progress`), ~~자율 분해/재계획/discovery serve 배선~~ ✅(AUTONOMY: `goal_scheduler_tick` serve 배선·자율 분해, `planning.replan`/`approval_disposition`, continuation replan, 4-pass 전체 루프).
...
- **GW4-B physical-execution (apply path) — ✅ done (실제 bounded mutation + commit + evidence→vault):**
  `execute_bridge.apply_approved_packet(goal, mutator, repo_root, ...)`(= `execute_approved_packet(...,
  apply=True, mutator=...)`) 가 **opt-in** 실제 실행 경로다. **기본 경로
  `execute_approved_packet(goal, env=env)` 는 그대로 authorize+evidence only**(console approve / #348
  무변경) — `apply=False` 기본 + mutator 미주입이면 절대 물리 mutation 하지 않는다. apply 경로는
  bounded runtime/daemon(명시 caller) 전용. apply 시: (a) 3-gate(`run_internal_chain` +
  `can_specialist_execute` + `authorize_runtime_execution` + `validate_execution`) PASS, (b) class==safe,
  (c) `BoundedMutator` 존재일 때만 → BoundedMutator 로 hard-cap(diff/file)·재읽기 검증된 bounded write
  수행 → 성공 시 `git -C repo_root` 로 **실제 commit**(author=`git_author_for(executor)`,
  `Forgekit-Agent` trailer carry, sha 캡처, **push 절대 안 함**) → goal 에 `execution`+`verification`
  evidence(commit sha 포함) + vault_root 설정 시 Nexus vault 에 `write_note` 로 authored evidence note.
  caps 초과 / verify 실패 / risky / non-safe → **rollback**(git restore/파일 삭제), commit 없음, 정직한
  blocked outcome. **fake "executed/applied" 없음, 검증 evidence 없이 done 전이 절대 없음.** executor 는
  실제 `is_known` registry id(`backend-engineer`).
  - **vault 경계(정직):** `FORGEKIT_NEXUS_ROOT`(env) 또는 `config['nexus_root']` 가 실 디렉터리면 note 기록,
    미설정이면 goal-store evidence 만 남기고 vault write 는 **정직하게 skip**(fake note/에러 없음).
  - **acceptance:** `tests/forgekit/test_bounded_exec.py` 8 케이스(safe→real write+verify+real commit+
    trailer+goal evidence+vault note / vault 미설정→skip / risky·destructive→no write·no commit /
    caps 초과→rollback / verify 실패→rollback / 기본 경로 무변경 regression) — **TEMP git repo(tempfile)
    사용, 실 repo 절대 미사용**. evidence `examples/goal/bounded-exec.txt`.
- **GW4-B always-on caller (G1, axis 4 operator-visible) — ✅ done (apply path 에 호출자 배선):**
  직전까지 `apply_approved_packet` 는 **호출자가 없어** operator 가 `/goal` 로 승인한 packet 이 실제로는
  한 번도 물리 실행되지 않았다. 이제 `forgekit runtime serve` 의 tick 이 그 호출자다.
  `runtime/goal_exec_tick.py` `GoalExecTicker` 가 매 tick 에서 `GoalStore.load_all()` 로 **ACTIVE
  (operator-승인) goal** 만 읽어 linked packet 을 `apply_approved_packet(goal, BoundedMutator(repo_root),
  repo_root, ...)` 로 실행 → safe-class + 3-gate PASS 분만 실제 bounded write + 실 commit(push 안 함)
  + `execution`/`verification` evidence, risky/destructive/미인가 는 `decision` 거부 evidence 만(실행 0).
  `cli/runtime_cmd.py` `_build_tick_fn` 이 autopilot pass + **goal-exec pass** 를 합성해 `forgekit runtime
  serve` / `once` 가 이 실행에 실제로 도달. **operator 경로:** `/goal` 로 goal ACTIVE 승인 → `forgekit
  runtime serve` 가 safe packet 물리 실행 + evidence → `forgekit runtime status`/goal evidence 로 확인.
  - **bounded/idempotent:** 최대 2 goal × 2 packet/tick; 이미 `execution`(실행) 또는 `decision`(거부)
    evidence 가 있는 packet 은 다음 tick 에 재시도 안 함(중복 commit/거부 churn 방지, 재시작 후에도 durable).
  - **safety:** ACTIVE goal 만(awaiting_approval/blocked skip), safe-class 만(apply 강제), never push,
    never destructive, verified evidence 없이 `done` 절대 전이 안 함.
  - **acceptance:** `tests/forgekit/test_goal_exec_serve.py` 9 케이스(active-safe→실행+commit+evidence /
    awaiting·blocked→실행 안 함 / risky·destructive→gated-record / idempotent next-tick / serve `once`
    integration / `_build_tick_fn` 합성 경로) — **TEMP git repo(tempfile) 사용, 실 repo 절대 미사용**.
    evidence `examples/runtime/goal-exec-serve.txt` (+ `goal-exec-serve-README.md`).
- **honest 물리 mutation 경계(직전 단계 — orchestration+evidence):** 기본 `execute_approved_packet` 는
  자율로 diff/commit 을 만들지 않는다. 인가된 실행이 carry 할 **trailer 가 stamp 된 commit message**
  (`build_execution_commit_message`, `execution_commit_trailers` 기반, `Forgekit-Agent` = registry
  `is_known` id)를 빌더로 노출하고 인가+검증을 evidence 로 기록한다. 실제 물리 mutation 은 위 apply 경로
  (BoundedMutator)에서만 — 일어나지 않은 diff/commit 을 날조하지 않는다.
...
### GW-EXEC — goal decomposition + continuation loop (GW1·GW4 의존) — ✅ **done**
- 브랜치: `feat/forgekit-goal-exec-core` (main 위). **문제:** GW4-B 까지로 "승인된 packet→물리 실행"
  은 닫혔지만, **큰 goal 을 끝까지 닫는** 경로가 없었다 — goal 은 평면 packet 더미로만 쌓이고
  (`add_child` 호출자 0), step 진척/완료 rollup/다음 step 결정(continuation)이 없어 "goal 은 있는데
  실행 teeth 가 약함". GW-EXEC 가 그 gap 을 닫는다.
- **신규 `forgekit_goal/planning.py` (pure, executes nothing):**
  - `decompose(parent, [PlanStep,...])` — 큰 goal 을 **ordered child goal** 로 분해(각 child `parent_id`
    링크 + parent `add_child` + parent `plan` evidence 1건). plan 레코드 생성만 — 파일/packet 안 건드림
    (safe-class by construction).
  - `tally_packets` / `is_goal_complete` — **append-only evidence 에서 완료 도출**: 모든 proposal packet 이
    `execution` evidence 를 갖고 pending/gate-blocked 0 이며 `verification` 1건 이상일 때만 complete
    (packet 0 개는 complete 아님 — no fake-green, transitions `done` 가드와 동일 규칙).
  - `progress(goal, children)` — 분해된 parent 는 child 기준, leaf 는 packet 기준 진척 + `next_step_id`.
  - `continuation_action(parent, children)` — **다음 1개 legal move**: 다음 DRAFT child `ADVANCE` /
    blocked·gate-stuck child `REPLAN`(operator) / in-flight `WAIT` / 모든 child done `COMPLETE`. 적용은
    안 하고 caller 가 `transitions.apply` 로 라우팅(매트릭스+evidence 가드 유지). blocked step 을 절대
    auto-advance 하지 않음 → 승인 chain 우회 0.
- **신규 `runtime/goal_continuation_tick.py` `GoalContinuationTicker` (loop 닫기):** 매 tick 에서
  (1) packet 이 전부 verified 된 ACTIVE child 를 `active->done` 으로 rollup(evidence-gated, `verification`
  rollup 기록), (2) 분해된 parent 마다 `continuation_action` 적용 — 다음 DRAFT child `draft->active`(다음
  tick 에 exec tick 이 그 packet 실행) 또는 모든 child done 시 parent `active->done`. **실행 0**(goal status +
  evidence 만 — 물리 mutation 은 GW4-B exec tick/bridge 에). `cli/runtime_cmd.py` `_build_tick_fn` 에
  **3번째 pass** 로 합성(continuation 이 마지막 — exec pass 가 이번 tick 에 쓴 execution evidence 를 같은
  tick 에 rollup). bounded(최대 4 parent/tick) + idempotent(done child/parent 재tick=no-op).
- **신규 surface:** `/goal plan <id> s1 | s2 | s3`(operator 분해, child goal 생성, 실행 0, 재분해 가드),
  `/goal progress <id>`(derived 진척 + 다음 continuation action), `/goal show` 에 step tree.
- **operator 경로:** `/goal new` → `/goal plan` 으로 step 분해 → `/goal activate` (parent ACTIVE) →
  `forgekit runtime serve` 가 continuation 으로 step 순차 activate + exec 으로 safe packet 실행 +
  continuation 으로 rollup → 모든 step 완료 시 goal `done`. risky step 은 `awaiting_approval`/REPLAN 으로
  operator 에게 표면.
- **acceptance:** `tests/forgekit/test_goal_planning.py`(14, pure decompose/progress/continuation +
  no-fake-green) + `test_goal_continuation_serve.py`(6, decompose→exec→verify→rollup→done 전체 lifecycle /
  evidence-gated(미실행 시 done 안 됨) / blocked→REPLAN(승인 우회 0) / idempotent / `_build_tick_fn` 합성
  end-to-end — **TEMP git repo 사용, 실 repo 미사용**) + `test_goal_surface.py` plan/progress 4 케이스.
  evidence `examples/goal/planning.txt`(+ `_regen_planning.py`, deterministic).
- **honest boundary:** decompose 는 plan 레코드 생성만(safe), continuation 은 status+evidence 만(실행 0),
  parent `done` 은 모든 child 가 real execution+verification evidence 보유 시에만(evidence-gated),
  blocked/risky step 은 REPLAN/awaiting 으로 operator 에게 — 승인 chain 우회·fake automation·fake done 0.

### AUTONOMY — 자율 분해 + 재계획 + 전체 bounded 루프 배선 (GW-EXEC 의존) — ✅ **done**
- 브랜치: `feat/forgekit-goal-autonomy-core` (main 위). **문제:** GW-EXEC 까지로 *operator-driven*
  decompose(`/goal plan`) + continuation 은 닫혔지만, ForgeKit 이 **스스로** 큰 goal 을 쪼개고
  막히면 재계획하는 autonomy 가 없었다. 결정적 gap: **discovery(`goal_tick`) 가 serve 루프에 미배선** —
  ACTIVE goal 이 always-on 에서 packet 을 한 번도 자동 수집하지 못했다("collect→choose packet" 단계 부재).
- **신규 `runtime/goal_scheduler_tick.py` `GoalSchedulerTicker` (루프의 FRONT, 실행 0):** 매 tick 에서
  work 없는 ACTIVE leaf goal 에 discovery(`run_self_improvement`)를 돌려 packet 수집+링크(`goal_tick.
  link_packets`, proposal evidence). 발견 work 가 **≥2 area 면 자율 분해**(`planning.is_big_goal` →
  `derive_plan_steps` → `decompose`): area 당 child goal 1개 + 각 area packet 을 child 로 라우팅("큰 goal 이면
  무조건 packetized execution"). 단일 area 면 leaf 유지. risky packet 은 즉시 `awaiting_approval` 로 park
  (approval-needed split). bounded(max 2 goal/tick) + idempotent(proposal evidence/children 있으면 skip,
  단 replan 된 goal 은 1회 재구동). discovery 는 injectable(hermetic 테스트).
- **신규 `planning` replan policy + 분류 (pure):** `replan(goal, max_attempts)` — stuck(gate-blocked packet,
  pending 0, 미완료)면 bounded RETRY(죽은 packet **unlink** → 재구동으로 대안 모색, gate-refused packet
  **재실행 안 함**) 후 소진 시 ESCALATE(`blocked` evidence 로 사유 영속 + active→awaiting_approval).
  `is_stuck`/`blocked_reason`(최신 거부 사유 영속) + `approval_disposition`(pending packet risk 태그로
  NEEDS_APPROVAL vs AUTONOMOUS_SAFE — unknown=안전거부로 needs_approval). `tally_packets` 가 `goal.packets`
  교집합으로 바뀌어 unlink 가 회계에서 빠짐.
- **continuation tick 확장:** REPLAN 을 실제 적용 — stuck ACTIVE leaf goal(standalone/child 모두) 에
  `replan` 호출 → RETRY(unlink+replan evidence) 또는 ESCALATE(blocked evidence+awaiting). `_build_tick_fn`
  은 이제 **4 pass**: autopilot → **scheduler(collect/decompose)** → exec → continuation(rollup/advance/replan).
  scheduler 가 FIRST(갓 활성화된 goal 을 exec 전에 packetize), continuation LAST(같은 tick exec evidence rollup).
- **surface:** `/goal progress` 에 approval disposition(자율 안전/승인 필요) + 막힘 사유, `/goal show` 에 막힘 사유.
- **각 tick 루프(필수 작업 #3 충족):** collect state(scheduler discovery) → choose+classify packet(link_packets
  risk 태그) → execute or await(exec tick safe 실행 / risky→awaiting) → verify(exec verification) → write
  evidence(execution/verification/proposal/replan/blocked) → 막히면 replan/escalate.
- **acceptance:** `test_goal_planning.py`(+13=27: disposition/replan/derive) + `test_goal_scheduler_serve.py`(6:
  leaf packetize / 자율 분해 by area / risky→awaiting / idempotent / draft skip / **full autonomous loop:
  activate big goal→scheduler decompose→exec→verify→continuation→done**) + `test_goal_continuation_serve.py`
  (replan RETRY/ESCALATE) + `test_goal_surface.py`(disposition/blocked reason). evidence `examples/goal/
  autonomy.txt`(+ `_regen_autonomy.py`, deterministic).
- **honest boundary:** scheduler/replan/decompose 는 record(proposal/plan/replan/blocked evidence + child goal)
  생성만 — **실행 0**(물리 mutation 은 exec tick/bridge gate). risky=awaiting(승인 우회 0), stuck=bounded
  retry→escalate(gate-refused packet 재실행 0, fake progress 0), done=evidence-gated(미실행 시 done 0).
...
## 9. goal evidence → Nexus 축 (continuity)

goal progression 의 packet/evidence 가 **실제 Nexus artifact 로 누적**되는 축은
[`docs/nexus-evidence-axis.md`](nexus-evidence-axis.md) 가 SSoT. `/goal publish <id>` 가 goal
evidence 를 schema 고정(goal_id/lane/packet_id/role/status/created_at/evidence_path) authored
note 로 mirror 하고, `nexus-note` 레코드(`ref`=노트 경로)를 goal 에 append 해 `/goal evidence`
가 실제 artifact 증가를 반영한다. future external collection 은 동일 스키마(`lane=discovery`)로
같은 축에 붙는다(`nexus.vault.discovery_intake_meta`).
</content>
</invoke>
