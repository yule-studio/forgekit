...
| 4 | Claude Code 근접 TUI | 🟡 substantial | `tui/` ~6390 LOC: composer/palette/transcript/process feed 존재. copy/paste/attach(image staging) surface 확인됨(`H_COPY`/`H_PASTE`/`H_ATTACH`). `/goal` surface 추가(GW5). **남은 gap:** in-console approve/deny UI 없음(데몬 상태만 표시). |
| 5 | `/goal` 중심 자기관리(수집·분석·실행·검증·기록) | 🟡 core+tick+surface done (GW1·GW4-A·GW5) | GW1 모델/영속 + GW4-A goal-tick(수집→제안→linkage→evidence, 실행 0) + GW5 `/goal` console surface(list/new/show/activate/evidence) ✅. evidence `examples/goal/{roundtrip,tick,surface}.txt`. **남은 것:** 승인된 packet→autopilot 실행 bridge(GW4-B seam). |
...
실행 순서(의존 기준): **GW1 ✅ → GW2-A ✅ → GW3 ✅ → GW4-A ✅ → GW6 ✅ → GW5 ✅ → GW7(다음) → GW8** (+ GW4-B 실행 bridge seam, in-console approve/deny UI seam).
...
- **TUI parity 현황(정직):** copy/paste/attach(image staging) surface 는 이미 존재(`H_COPY`/`H_PASTE`/
  `H_ATTACH`), palette/transcript/process feed 도 존재. **남은 gap = in-console approve/deny UI**
