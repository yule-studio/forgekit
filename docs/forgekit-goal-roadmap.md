...
| 5 | `/goal` 중심 자기관리(수집·분석·실행·검증·기록) | ✅ core+tick+surface+execute+**decompose/continuation** done (GW1·GW4-A·GW4-B·GW5·GW-EXEC) | GW1 모델/영속 + GW4-A goal-tick(수집→제안→linkage→evidence, 실행 0) + GW4-B execute bridge(승인된 packet→기존 게이트 실행 인가→execution+verification evidence) + GW5 `/goal` console surface + **GW-EXEC `forgekit_goal.planning`(큰 goal→ordered child step 분해 + evidence-derived progress + sequential continuation: 다음 step advance / step 완료 rollup / 모든 step 완료 시 parent evidence-gated done)** ✅. evidence `examples/goal/{roundtrip,tick,surface,execute-bridge,planning}.txt`. **남은 것:** 실제 물리 mutation 은 BoundedMutator 게이트(WT3, P2). |
...
| 9 | 새 기능/패턴/provider capability 자기조사→backlog 승격 | 🟡 promotion path done (GW4-A) + **plan 분해(GW-EXEC)** | discovery/repo-local 신호 → risk-classified packet → **goal 에 linkage + evidence 승격**(`goal_tick.tick_goal`, GW1 연결) ✅. **GW-EXEC:** 큰 goal 을 `/goal plan` 으로 ordered child step 으로 분해 → continuation tick 이 step 을 순차 실행·rollup·완료(`planning` + `goal_continuation_tick`). **남은 것:** 외부 connector(Figma/YouTube=P2). |
...
실행 순서(의존 기준): **GW1 ✅ → GW2-A ✅ → GW3 ✅ → GW4-A ✅ → GW4-B ✅ → GW6 ✅ → GW5 ✅ → GW7 ✅ → GW8 ✅ → GW-EXEC ✅** (merge-prep: [`forgekit-goal-mergeprep.md`](forgekit-goal-mergeprep.md)). 남은 seam: ~~GW4-B 실행 bridge~~ ✅(`execute_approved_packet`, 기존 게이트 재사용·execution+verification evidence·trailer stamp), ~~GW4-B 실제 물리 mutation~~ ✅(`apply_approved_packet` opt-in: safe-class 만 BoundedMutator bounded write + 실제 git commit + evidence→Nexus vault, 기본 경로 무변경; destructive 는 여전히 approval-gated), ~~in-console approve/deny UI~~ ✅(`/goal awaiting|approve|deny`, wave-2 gw3), ~~GW2-B trailer→identity~~ ✅(`check_identity_binding`, bridge commit message 에 trailer 자동 stamp), ~~큰 goal 분해/continuation(실행 teeth)~~ ✅(GW-EXEC: `forgekit_goal.planning` decompose+progress+continuation + `goal_continuation_tick` rollup→evidence-gated done, `/goal plan|progress`).
