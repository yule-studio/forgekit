...
| 3 | gateway→tech-lead→specialist 승인/분배 **런타임 강제** | 🟡 contract-only (검증 완료) | `apps/engineering-agent/.../council.py` 는 **타입 SSoT 만** (docstring: "런타임 로직 router/runner/approval surface 은 본 PR scope 가 아니다"). 8 deliverable schema(`ApprovalPacket` 등) + enum + frozen dataclass 존재, **런타임 강제 없음**. GW3 이 enforcement 를 신설. |
...
| 5 | `/goal` 중심 자기관리(수집·분석·실행·검증·기록) | 🟡 core done (GW1) | `packages/forgekit-goal` 신설: 모델/전이/evidence/linkage/영속 ✅(18/18, `examples/goal/roundtrip.txt`). **남은 것:** tick/실행 루프(GW4) + `/goal` console surface(GW5). |
...
| 7 | commit/identity/authorship/approval **코드 강제** | 🟡 partial | `forgekit_config.identity` (git author/app) working(`/whoami`). **gap:** commit-message template/trailer validator + approval metadata + **CI guard** 미확인/미강제. GW2. |
...
| 9 | 새 기능/패턴/provider capability 자기조사→backlog 승격 | 🟡 partial | discovery collectors(free-first) working. **gap:** 조사 결과를 **goal/packet backlog 로 승격**하는 경로 없음 — GW4(self-improvement teeth)가 GW1(`/goal`)에 연결돼야 성립. |
...
실행 순서(의존 기준): **GW1 → GW2 → GW3 → GW4 → GW5 → GW6 → GW7 → GW8.**
...
- 브랜치: `feat/forgekit-commit-governance`. 위치: `packages/forgekit-config` + `.github/workflows`.
- commit template(`policies/reference/COMMIT_CONVENTION.md` 기반) + trailer(agent identity/approval
  metadata) validator. **Co-Authored-By 금지** ([[feedback_commit_format]]).
- agent identity → git author/committer 매핑 강제(`fk-<role>`, [[project_forgekit_agent_identity_ssot]]).
- **acceptance:** validator 단위 테스트(정상/위반 케이스) + CI guard workflow 가 위반 commit 을
  fail 시킴(로컬 재현 가능한 dry-run). evidence: 위반/통과 샘플.
- **honest boundary:** 기존 `git_path_safety` hard rail 과 중복 금지 — commit *메시지/trailer* 만 담당.

### GW3 — approval chain 런타임 강제
- 브랜치: `feat/forgekit-approval-chain`. 먼저 `council.py` 가 ForgeKit runtime 에 실제로 물려
  있는지 **코드 확인**(없으면 그 사실을 honest 하게 doc + 신설). 분배: gateway(operator surface)
  → tech-lead(technical approval) → specialist(실행).
- **acceptance:** submit/packet 실행이 체인 미통과 시 거부됨을 보이는 테스트 + 우회 불가 증명.
- **honest boundary:** 문서기준 done 으로 적힌 게 코드와 다르면 reality matrix 를 정직하게 수정.

### GW4 — always-on self-improvement teeth (GW1·GW3 의존)
- 브랜치: `feat/forgekit-goal-selfimprove`. `forgekit_runtime.selfimprove` + `autopilot` 를
  GW1 goal 에 연결: **goal tick → source collection(Nexus/discovery) → packet proposal
  (Hephaistos) → approval wait(notify inbox) → bounded execute → verify → evidence write**.
- **acceptance:** `test_goal_tick_loop` — 한 tick 이 goal 을 읽고 packet 을 제안→inbox 에 승인
  대기로 멈춤(승인 없이 실행 0) → 승인 주입 시 safe-class 실행→evidence append. P1 teeth = bounded.
- **honest boundary:** destructive 는 approval-gated. 승인 없는 자율 실행 절대 금지(#8).
