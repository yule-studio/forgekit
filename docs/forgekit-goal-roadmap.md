...
| 5 | `/goal` 중심 자기관리(수집·분석·실행·검증·기록) | ✅ core+tick+surface+execute+decompose/continuation+**autonomy(자율분해/재계획)** done (GW1·GW4·GW5·GW-EXEC·AUTONOMY) | GW1 모델/영속 + GW4-A goal-tick(수집→제안→linkage→evidence, 실행 0) + GW4-B execute bridge(승인된 packet→기존 게이트 실행 인가→execution+verification evidence) + GW5 `/goal` console surface + GW-EXEC `forgekit_goal.planning`(큰 goal→ordered child step 분해 + progress + continuation) + **AUTONOMY `goal_scheduler_tick`(serve 에 discovery 배선: ACTIVE goal 자동 packetize + ≥2 area 면 자율 분해) + replan policy(stuck→bounded retry→escalate, 사유 영속) + approval disposition(자율안전/승인필요 분리), `_build_tick_fn` 4-pass 전체 루프)** ✅. evidence `examples/goal/{roundtrip,tick,surface,execute-bridge,planning,autonomy}.txt`. **남은 것:** 실제 물리 mutation 은 BoundedMutator 게이트(WT3, P2). |
