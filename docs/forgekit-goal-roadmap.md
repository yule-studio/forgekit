...
- **G1 — ✅ done (always-on serve 루프가 goal 을 실제로 굴림 = goal-driven execution continuity):**
  직전 단계까지 `apply_approved_packet` 의 **caller 가 0**(seam) — goal-tick 은 packet/evidence 만 만들고
  멈췄고, 물리 실행은 console `/goal approve`(record-only) 로만 닿았다. `runtime/goal_exec_tick.py`
  (`execute_active_goals`)가 그 경계를 닫는다: 매 bounded tick 마다 ACTIVE goal 의 **다음 pending safe
  packet** 을 `apply_approved_packet`(BoundedMutator, 위 apply 경로)로 물리 실행 → execution/verification
  evidence(goal store + vault). `AutopilotTicker`(serve tick)가 이를 매 tick 구동(`execute_goals=True`
  기본) — `forgekit runtime serve` 가 goal-DRIVEN 이 된다(repo-finding 전용 아님). 정직 경계: risky/
  destructive proposal 은 goal 을 `awaiting_approval` 로 두고 **자동 실행 안 함**(operator `/goal approve`
  전까지), self-gate 로 ACTIVE+risky 도 OUTCOME_BLOCKED. dedupe(실행된 packet 재실행 안 함)·bounded(틱당
  goal 상한)·stoppable(kill-switch)·inspectable(evidence + TickOutcome). 즉 **host uptime 이 아니라 goal
  이 살아있는 동안 다음 safe step 을 계속 찾아 승인 체계 아래 실행하고 evidence 를 누적**한다. acceptance
  `tests/forgekit/test_goal_exec_tick.py`(9: 물리 실행 / risky→awaiting / forced-active-risky→gate-blocked
  / draft 제외 / dedupe / serve-tick 구동 / execute_goals=off no-op), evidence `examples/goal-continuity/`.
