...
`/resolve <요청>` 은 **미리보기**(persist=False) — 게이트 결과만 보여주고 기록하지
않는다. operator 가 실제로 기록·실행하려면 subverb 를 쓴다 (G2):

- **`/resolve apply <요청>`** — 게이트를 실행하고 receipt 를 append-only ledger 에
  **persist**. 출력에 receipt + 기록된 ledger 경로를 보여준다 (validation 통과분만
  기록 — fake 거부). 코드: `router._forge_subverb_result`.
- **`/resolve log`** — 최근 ledger 기록을 되읽어 audit 로 보여준다 (✓ executed /
  ✗ blocked — destructive/blocked 도 정직하게 기록·표시).

→ "실행 전/후에 무엇이 인가됐는지" 가 operator-visible artifact 로 남고, 영속된다.
