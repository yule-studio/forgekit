...
def test_bootstrap_persists_task_brief_and_role_work_orders() -> None:
...
def test_bootstrap_sets_lifecycle_substage_within_council_vocabulary() -> None:
    bootstrap = bootstrap_council(
        session_id="sess-002",
...
def test_three_seat_bootstrap_produces_owner_challenger_reviewer() -> None:
...
def test_ready_for_synthesis_blocked_without_settled_peer_review() -> None:
    brief = build_task_brief(
        session_id="sess-004",
        title="X",
        purpose="P",
...
def test_determine_substage_ready_when_settled() -> None:
...
def test_determine_substage_escalated_after_round_cap() -> None:
...
def test_determine_substage_round_complete_when_active_under_cap() -> None:
...
def test_public_summary_fallback_includes_role_round_status() -> None:
...
def test_public_summary_returns_existing_when_non_blank() -> None:
    summary = ensure_public_summary(
        "이미 채워진 본문",
...
def test_council_result_public_summary_filled_even_with_no_evidence() -> None:
...
def test_already_bootstrapped_false_for_legacy_session_extra() -> None:
    assert already_bootstrapped({}) is False
    assert already_bootstrapped({"active_research_roles": ["tech-lead"]}) is False


def test_synthesis_gate_returns_reason_when_council_keys_missing() -> None:
...
def test_persist_bootstrap_skips_when_session_is_none() -> None:
...
def test_bootstrap_is_idempotent_when_extras_already_present() -> None:
    session = _FakeSession(
        session_id="sess-008",
        extra={
...
def test_role_take_adapter_preserves_four_section_contract() -> None:
...
def test_role_take_adapter_batch_threads_provider_per_role() -> None:
    takes = [
...
def test_role_councils_round_trip_through_extras() -> None:
...
def test_synthesis_gate_returns_none_after_full_bootstrap() -> None:
    bootstrap = bootstrap_council(
...
def test_router_glue_runs_when_active_roles_present() -> None:
...
def test_router_glue_no_op_when_session_is_none() -> None:
    from yule_orchestrator.discord.engineering_channel_router.council_flow import (
        maybe_bootstrap_council,
    )
...
def test_router_glue_falls_back_to_tech_lead_when_no_active_roles() -> None:
    from yule_orchestrator.discord.engineering_channel_router.council_flow import (
        maybe_bootstrap_council,
    )
...
def test_router_glue_skips_when_already_bootstrapped() -> None:
    from yule_orchestrator.discord.engineering_channel_router.council_flow import (
        maybe_bootstrap_council,
    )
...
def test_required_outputs_for_role_returns_role_specific_keys() -> None:
...
def test_fallback_public_summary_handles_blank_role() -> None:
