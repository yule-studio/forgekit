...
def test_advance_council_round_recomputes_round_two_from_unsettled_state() -> None:
...
def test_advance_council_round_returns_none_when_brief_missing() -> None:
...
def test_advance_council_round_accepts_short_role_input() -> None:
...
def test_round_two_without_settlement_escalates_at_cap() -> None:
    extra = _make_unsettled_round_one_extra("backend-engineer")
    advanced = advance_council_round(
        extra,
        role="backend-engineer",
...
def test_must_escalate_to_tech_lead_aligns_with_substage() -> None:
...
def test_disagreement_summary_filled_when_round_unsettled() -> None:
...
def test_disagreement_summary_filled_when_escalated() -> None:
    brief = build_task_brief(session_id="x", title="X", purpose="P")
...
def test_ensure_disagreement_summary_does_not_inject_for_settled() -> None:
...
def test_council_state_missing_emits_when_active_roles_set_but_council_absent() -> None:
...
def test_council_signals_silent_for_legacy_session_without_council_keys() -> None:
...
def test_council_bootstrap_error_signal_when_router_stamped_reason() -> None:
...
def test_council_signals_skipped_when_intake_not_completed() -> None:
    extra = {COUNCIL_BOOTSTRAP_ERROR_EXTRA_KEY: "should be ignored"}
    assert collect_council_signals(extra, intake_completed=False) == ()


def test_router_glue_stamps_bootstrap_error_when_no_active_roles() -> None:
...
def test_canonical_role_and_short_role_round_trip() -> None:
...
def test_normalize_roles_collapses_short_and_canonical_into_canonical() -> None:
    out = normalize_roles(
        [
            "backend-engineer",
            "engineering-agent/backend-engineer",
...
def test_bootstrap_treats_short_and_canonical_input_consistently() -> None:
    a = bootstrap_council(
        session_id="x",
        canonical_prompt="P",
...
def test_provider_seat_matrix_uses_manifest_preferred_advisors() -> None:
...
def test_provider_seat_matrix_falls_back_when_manifest_unavailable() -> None:
...
def test_bootstrap_stamps_provider_seat_matrix_on_session_extra() -> None:
...
def test_settled_bootstrap_still_emits_info_ready_signal() -> None:
    result = bootstrap_council(
        session_id="legacy",
...
def test_settled_bootstrap_substage_unchanged_from_c2() -> None:
    result = bootstrap_council(
        session_id="legacy-2",
        canonical_prompt="settled case",
...
def test_round_one_unsettled_emits_round_2_pending_signal() -> None:
    extra = _make_unsettled_round_one_extra("backend-engineer")
    sigs = collect_council_signals(extra)
    codes = [s.code for s in sigs]
...
def test_escalation_signal_includes_disagreement_summary_when_digest_present() -> None:
    extra = _make_unsettled_round_one_extra("backend-engineer")
    advanced = advance_council_round(extra, role="backend-engineer")
    assert advanced is not None
...
def test_advance_council_for_role_updates_session_extra() -> None:
...
def test_advance_council_for_role_stamps_error_on_missing_brief() -> None:
...
def test_advance_council_for_role_noop_when_session_is_none() -> None:
