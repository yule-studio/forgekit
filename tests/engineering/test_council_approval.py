...
def test_settled_council_and_signoff_yield_packet() -> None:
...
def test_packet_payload_round_trips_through_extras() -> None:
...
def test_escalated_council_blocks_packet_creation() -> None:
...
def test_packet_blocked_when_no_council_history() -> None:
...
def test_conditional_signoff_yields_conditional_packet() -> None:
    extra = _make_settled_session_extra("backend-engineer")
    signoff = TechLeadSignoff(
...
def test_packet_blocked_when_signoff_missing() -> None:
    extra = _make_settled_session_extra("backend-engineer")
...
def test_packet_blocked_when_signoff_status_blocked() -> None:
    extra = _make_settled_session_extra("backend-engineer")
    signoff = TechLeadSignoff(
...
def test_packet_blocked_when_signoff_status_escalated() -> None:
    extra = _make_settled_session_extra("backend-engineer")
    signoff = TechLeadSignoff(
...
def test_apply_signoff_only_advances_substage_for_settled_statuses() -> None:
    extra = _make_settled_session_extra("backend-engineer")
    # CONDITIONAL → advance
...
def test_aggregate_escalations_returns_per_role_summary() -> None:
...
def test_aggregate_uses_latest_round_per_role() -> None:
...
def test_advance_council_round_stamps_multi_role_aggregate() -> None:
...
def test_refresh_escalation_aggregate_silent_when_no_escalation() -> None:
    extra = _make_settled_session_extra("backend-engineer")
...
def test_gateway_surface_payload_starts_with_technical_then_operator() -> None:
    extra = _make_settled_session_extra("backend-engineer")
    signoff = TechLeadSignoff(
        status=TechLeadSignoffStatus.CONDITIONAL,
        rationale="회귀 ≥3 추가",
...
def test_signed_off_packet_has_no_operator_line_when_no_external_actions() -> None:
    extra = _make_settled_session_extra("backend-engineer")
    signoff = TechLeadSignoff(
        status=TechLeadSignoffStatus.SIGNED_OFF, rationale="OK"
    )
...
def test_post_gateway_surface_advances_substage_and_stamps_payload() -> None:
    extra = _make_settled_session_extra("backend-engineer")
    signoff = TechLeadSignoff(
        status=TechLeadSignoffStatus.SIGNED_OFF, rationale="OK"
    )
...
def test_persist_role_selection_normalises_to_canonical_form() -> None:
...
def test_normalize_roles_strips_duplicates_across_forms() -> None:
    out = normalize_roles(
...
def test_build_provider_availability_marks_unavailable_candidates() -> None:
...
def test_bootstrap_stamps_availability_when_available_providers_supplied() -> None:
...
def test_bootstrap_omits_availability_when_not_supplied() -> None:
    bootstrap = bootstrap_council(
        session_id="s",
        canonical_prompt="P",
        active_roles=["backend-engineer"],
...
def test_settled_bootstrap_still_lands_on_ready_for_synthesis_substage() -> None:
...
def test_status_signals_still_emit_ready_for_settled_bootstrap() -> None:
    bootstrap = bootstrap_council(
        session_id="legacy",
        canonical_prompt="P",
        active_roles=["backend-engineer"],
...
def test_status_signal_packet_drafted_after_signoff_and_packet_draft() -> None:
    extra = _make_settled_session_extra("backend-engineer")
    signoff = TechLeadSignoff(
        status=TechLeadSignoffStatus.SIGNED_OFF, rationale="OK"
    )
...
def test_status_signal_surface_posted_after_post_gateway() -> None:
    extra = _make_settled_session_extra("backend-engineer")
    signoff = TechLeadSignoff(
        status=TechLeadSignoffStatus.SIGNED_OFF, rationale="OK"
    )
...
def test_status_signal_signoff_blocked_when_signoff_blocked() -> None:
    extra = _make_settled_session_extra("backend-engineer")
...
def test_router_glue_full_pipeline_stamps_packet_and_surface() -> None:
...
def test_router_draft_packet_stamps_bootstrap_error_when_signoff_missing() -> None:
    from yule_orchestrator.discord.engineering_channel_router.council_flow import (
        draft_approval_packet_for_session,
        maybe_bootstrap_council,
    )
