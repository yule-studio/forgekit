...
class LifecycleSubstageVocabularyTests:
    pass


def test_council_substage_vocabulary_has_expected_buckets() -> None:
...
def test_lifecycle_substage_extra_key_is_stable() -> None:
...
def test_synthesis_blocked_when_no_council_results() -> None:
    assert ready_for_synthesis([]) is False
    assert synthesis_block_reason([]) == "no_role_councils_recorded"


def test_synthesis_blocked_when_any_role_escalated() -> None:
...
def test_synthesis_allowed_when_all_settled_even_with_conditions() -> None:
    a = _make_council_result(
        role="engineering-agent/backend-engineer",
        round_index=1,
        status=CouncilConsensusStatus.AGREED,
...
def test_synthesis_uses_latest_round_per_role() -> None:
...
def test_escalation_triggers_after_round_cap_without_agreement() -> None:
    history = [
...
def test_no_escalation_when_latest_round_settled() -> None:
    history = [
        _make_council_result(
            role="engineering-agent/backend-engineer",
            round_index=1,
...
def test_no_escalation_for_empty_history() -> None:
...
def test_packet_blocked_without_signoff() -> None:
    council = [
        _make_council_result(
            role="engineering-agent/backend-engineer",
            round_index=1,
...
def test_packet_blocked_when_signoff_is_blocked() -> None:
    council = [
        _make_council_result(
            role="engineering-agent/backend-engineer",
            round_index=1,
...
def test_packet_blocked_when_council_still_escalated() -> None:
    council = [
        _make_council_result(
            role="engineering-agent/backend-engineer",
            round_index=2,
...
def test_packet_creatable_when_signoff_and_council_settled() -> None:
    council = [
        _make_council_result(
            role="engineering-agent/backend-engineer",
            round_index=1,
...
def test_packet_creatable_when_signoff_conditional() -> None:
...
def test_review_loop_re_exports_execution_review_decision() -> None:
...
def test_execution_review_decision_vocabulary_is_stable() -> None:
...
def test_council_internal_actions_are_request_side() -> None:
    for action in (
...
def test_default_seats_are_three() -> None:
...
@pytest.mark.skip(reason="C2 phase: substage transition runner not implemented yet")
def test_TODO_lifecycle_substage_transition_runner() -> None:
...
@pytest.mark.skip(reason="C4 phase: gateway operator surface separation runner not landed")
def test_TODO_gateway_does_not_post_card_until_tech_lead_signoff() -> None:
    """C4 PR — `tech_lead_signoff.status != SIGNED_OFF / CONDITIONAL` 인
    동안에는 gateway 가 `#승인-대기` 카드를 게시하지 않는다."""


@pytest.mark.skip(reason="C5 phase: execution_review reopen wiring not landed")
def test_TODO_review_feedback_routes_to_reopen_or_review_loop() -> None:
