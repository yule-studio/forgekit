...
_COMMANDS: Tuple[SlashCommand, ...] = (
    ...
    SlashCommand("goal", "장기 목표 control plane — `/goal [list|new <제목>|show <id>|activate <id>|evidence <id>|publish <id>|awaiting|approve <id> [메모]|deny <id> [메모]]` (forgekit_goal 영속, publish=goal evidence 를 Nexus evidence 축에 schema 고정 authored note 로 기록, 승인은 awaiting_approval→active/blocked + decision evidence, tick/실행은 runtime/GW4)", H_GOAL, "status"),
