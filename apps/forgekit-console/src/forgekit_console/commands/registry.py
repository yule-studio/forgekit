...
_COMMANDS: Tuple[SlashCommand, ...] = (
    ...
    SlashCommand("discovery", "discovery 누적 루프 — free-first 수집→idea brief→**ledger 누적**(새 vs 추적중, lifecycle)+operator digest(왜/다음 질문). `/discovery [pending | candidates | evidence | promote <n> | save <n> | park <n>]` (pending=결정대기 목록, candidates=교차 관측·신선도 통과한 '물어볼 후보', evidence=경쟁gap·self-improve 신호를 vault evidence note 로 영속, promote=PM handoff 제안, save=연결 vault authored note, park=보류)", H_DISCOVERY, "status"),
