...
def _agents_dir() -> Path:
    # role_profiles.py lives at apps/engineering-agent/src/yule_engineering/agents/, so the repo
    # root is three parents up. Resolved once per call (cheap) so tests
    # that move CWD don't break path lookup.
    return Path(__file__).resolve().parents[5] / "agents" / "engineering-agent"
