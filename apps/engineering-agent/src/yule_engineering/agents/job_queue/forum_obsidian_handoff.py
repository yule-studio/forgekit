...
# ---------------------------------------------------------------------------
# Intake / persistence helpers — extracted to sibling modules along the
# ``intake / routing / persistence`` split axis. Imported here (after the
# orchestrator definitions, incl. ``_short_error``) so the call sites above
# keep working and the names stay re-exported for in-module references.
# One-way dependency: those modules import from this one, never vice-versa
# at call time — so placing the import here avoids an import-time cycle.
# ---------------------------------------------------------------------------

from .forum_obsidian_intake import (  # noqa: E402
...
from .forum_obsidian_persistence import (  # noqa: E402
    _emit_research_log_auto_save,
    _persist_ledger,
    _record_handoff_audit,
