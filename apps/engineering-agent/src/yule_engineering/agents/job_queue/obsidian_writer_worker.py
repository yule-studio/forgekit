...
# ---------------------------------------------------------------------------
# Default render dispatcher (research / decision / knowledge + A-M10b
# autonomous kinds) — extracted to :mod:`obsidian_writer_format` as a
# pure formatting split. :func:`default_render_fn` is re-exported from
# this module (see bottom of file) so every existing
# ``from .obsidian_writer_worker import default_render_fn`` keeps
# working. The re-export lives after :class:`ObsidianRenderError` is
# defined to avoid an import-time cycle (the format module imports
# ObsidianRenderError + the request type + note-kind constants from
# here, one-way).
# ---------------------------------------------------------------------------
...
# Re-export the default render dispatcher from its new home. Placed
# after every definition above (notably ObsidianRenderError) so the
# format module — which imports those symbols from here — sees a fully
# initialised module and no import-time cycle occurs.
from .obsidian_writer_format import default_render_fn  # noqa: E402
