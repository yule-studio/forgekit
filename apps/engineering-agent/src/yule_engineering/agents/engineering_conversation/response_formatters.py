...
def build_engineering_conversation_response(
    ...
    *collector_config* / *collector* are injection seams for tests and
    ...
    try:
        from yule_engineering.agents.routing import is_bot_echo_phrase as _is_bot_echo
    ...
    if intent.intent_id == APPROVAL_ACTION:
        try:
            from yule_engineering.agents.routing import is_non_actionable_prompt as _approval_guard
        except Exception:  # noqa: BLE001
    ...
    try:
        from yule_engineering.agents.coding.coding_bootstrap import (
            STATUS_BYPASS,
