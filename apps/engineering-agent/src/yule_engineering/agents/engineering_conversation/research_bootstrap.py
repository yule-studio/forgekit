...
2. **ResearchPack candidate classification** —
...
The collection-aware intake body formatters
(``_format_intake_with_collection``, ``_format_coding_bootstrap_body``,
``_format_collection_announcement``, ``_format_collection_meta_block``)
were split out into ``research_bootstrap_format`` (split axis:
formatting) and are re-exported here for backward compatibility.
"""
...
from yule_engineering.agents.messaging.dispatcher import TaskType
...
def _maybe_run_auto_collect(
    *,
    ...
    # canonical predicate.
    try:
        from yule_engineering.agents.routing import is_non_actionable_prompt as _is_blocked
...
# Collection-aware intake body formatters now live in
# ``research_bootstrap_format`` (split axis: formatting). Re-exported below
# (after this module's own definitions) so ``__all__`` and existing
# importers stay unchanged. The format module never imports this one at
# import time, so there is no cycle.
from .research_bootstrap_format import (  # noqa: E402
