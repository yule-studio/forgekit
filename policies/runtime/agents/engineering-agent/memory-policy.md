...
1. ✅ **retrieval boost wiring** — `memory/retrieval.py` 의 `fetch_role_context` 가 `YULE_RETRIEVAL_BOOST_ENABLED`(기본 off) 뒤에서 본 절 boost 를 적용(`agents/harness/retrieval_boost.py`). indexer 가 canonical/reusable/status 를 `extra` 로 투영. 측정은 `agents/harness/token_benchmark.py` + `runs/token-efficiency/`.
