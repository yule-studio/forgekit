...
``yule_orchestrator.discord.*`` import path via ``sys.modules`` so that existing
importers keep resolving to the **same** module objects::

    from yule_orchestrator.discord.ui.formatter import ...
    from yule_orchestrator.discord.engineering_channel_router import main
    import yule_orchestrator.discord.bot._legacy
...
runtime (``yule_orchestrator.agents`` / ``.runtime`` — apps → monolith, acyclic)
and ``yule_orchestrator.planning`` (→ ``apps/planning-agent`` via shim, app → app
