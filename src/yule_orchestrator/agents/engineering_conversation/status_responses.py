...
def _safe_list_sessions(lister):
    ...
    if lister is None:
        try:
            from yule_orchestrator.agents.workflow_state import list_sessions as _list
...
def format_blocked_reason_response(
    ...
    try:
        from yule_orchestrator.agents.lifecycle.session_status import diagnose_session as _diagnose
...
def format_status_diagnostic_response(
    ...
    *is_member_bot_question* tilts the answer toward "멤버 봇들은 뭐 하고
    ...
    if isinstance(growth_ledger, list) and growth_ledger:
        try:
            from yule_orchestrator.agents.lifecycle.growth_ledger import summarize_for_status
