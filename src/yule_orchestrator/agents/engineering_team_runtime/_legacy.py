...
from yule_orchestrator.agents.workflow_state import WorkflowSession, load_session
...
def build_turn_plan(session: WorkflowSession) -> Tuple[TeamTurn, ...]:
    ...
    from yule_orchestrator.agents.lifecycle.role_selection import get_effective_active_roles
...
def _render_role_runtime_preface(
    *,
    ...
    try:
        from yule_orchestrator.agents.role_profiles import output_template_for_role
...
def _collect_role_research_pack(
    *, session: WorkflowSession, role: str
    ...
    try:
        from yule_orchestrator.agents.research.collector import auto_collect_or_request_more_input
...
def _load_session_for_record(session_id: str) -> Optional[WorkflowSession]:
    ...
    if not session_id:
        return None
    try:
        from yule_orchestrator.agents.workflow_state import load_session as _load
    except Exception:  # noqa: BLE001 - partial install fallback
        return None
    try:
...
def _load_synthesis_text_from_session_extra(session: WorkflowSession) -> Optional[str]:
    ...
    if not isinstance(raw, dict):
        return None
    try:
        from yule_orchestrator.agents.deliberation import render_synthesis, synthesis_from_dict
...
def _load_pack_from_session_extra(session: WorkflowSession) -> Any:
    ...
    if not isinstance(raw, dict) or not raw:
        return None
    try:
        from yule_orchestrator.agents.research.pack import pack_from_dict
    except Exception:  # noqa: BLE001
        return None
    try:
...
def synthesize_thread(
    ...
    *,
    ...
    # labels.
    from yule_orchestrator.agents.deliberation import assign_citation_ids
